...
## 1. 使用的情境說明 (Context)
在實作Assignment IV時，因為涉及 C 和 C++ 兩個版本的對照開發，我利用 VS Code 的「分割視窗」功能同時編輯 `hash_fn.c` 與 `hash_fn.cpp`。這幫助我確保兩套語言在邏輯處理（尤其是中平方法的字串擷取部分）保持一致。
...
## 3. 版本控制流程
我使用 VS Code 內建的Source Control視圖進行 Commit 管理。
